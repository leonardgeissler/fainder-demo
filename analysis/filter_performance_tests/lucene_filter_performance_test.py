...
def test_lucene_filter_performance(evaluator: QueryEvaluator, keyword: str):
    csv_path = Path(pytest.csv_log_path_lucene) # type: ignore
    start_time = time.perf_counter()
    result_without_filtering, _,_ = evaluator.executor.lucene_connector.evaluate_query(keyword)
    time_without_filtering = time.perf_counter() - start_time
    log_performance_csv(csv_path, keyword, time_without_filtering, 0, 0, result_without_filtering)
    ...
    for additional_filter_size in ADDITIONAL_FILTER_SIZES:
        ...
        start_time = time.perf_counter()
        results, _,_ = evaluator.executor.lucene_connector.evaluate_query(keyword, doc_ids)
        execution_time = time.perf_counter() - start_time
        log_performance_csv(csv_path, keyword, execution_time, additional_filter_size, filter_size, results)

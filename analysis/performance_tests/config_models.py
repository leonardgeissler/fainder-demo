...
class OptimizerConfig:
    ...
    keyword_merging: bool = field(
        default=True, metadata={"description": "Enable keyword merging optimization"}
    )
...
class QueryGenerationConfig:
    ...
    max_num_query_per_term_count_percentile_combinations: int = field(
        default=5,
        metadata={
            "description": "Maximum number of queries to generate per term count"
        },
    )
    min_num_terms_multiple_percentile_combinations: int = field(
        default=2,
        metadata={
            "description": "Minimum number of terms in multiple percentile combinations"
        },
    )
    max_num_terms_multiple_percentile_combinations: int = field(
        default=4,
        metadata={
            "description": "Maximum number of terms in multiple percentile combinations"
        },
    )
    max_num_query_per_term_count_multiple_percentile_combinations: int = field(
        default=5,
        metadata={
            "description": "Maximum number of queries to generate per term count in multiple combinations"
        },
    )
    ...
    max_num_mixed_terms_extended_with_fixed_structure: int = field(
        default=20,
        metadata={
            "description": "Maximum number of mixed terms with extended fixed structure"
        },
    )
    max_num_mixed_terms_with_fixed_structure_not: int = field(
        default=10,
        metadata={
            "description": "Maximum number of mixed terms with fixed structure not"
        },
    )
    ...
    num_of_ḱws_for_multiple_pp_combinations: int = field(
        default=5,
        metadata={
            "description": "Number of keywords for multiple percentile combinations"
        },
    )
...
class EngineConfig:
    ...
    cache_size: int = field(
        default=0, metadata={"description": "Cache size for the engine"}
    )
...
class EnginesConfig:
    ...
    scenarios: List[EngineConfig] = field(
        default_factory=lambda: [
            ...
            EngineConfig(
                name="parallel_prefiltering_engine",
                executor_type="THREADED_PREFILTERING",
            ),
        ],
...
class ExperimentConfig:
    ...
    enabled_tests: List[str] = field(
        default_factory=lambda: [
            ...
            "multiple_percentile_combinations_with_kwexpected_form_not_queries",
    ...
    num_runs: int = field(
        default=1,
        metadata={
            "description": "Number of times to run each query for more reliable timing"
        },
    )
...
class PerformanceConfig:
    ...
    # Paths for logs
    log_dir: str = field(
        default="logs", metadata={"description": "Base directory for logs"}
    )
    performance_log_dir: str = field(
        default="logs/performance",
        metadata={"description": "Directory for performance logs"},
    )
    profiling_log_dir: str = field(
        default="logs/profiling",
        metadata={"description": "Directory for profiling logs"},
    )

    # Execution settings
    disable_profiling: bool = field(
        default=True, metadata={"description": "Whether to disable profiling"}
    )
    use_json_test_cases: bool = field(
        default=False, metadata={"description": "Whether to use JSON test cases"}
    )

    # Test configuration
    keywords: KeywordConfig = field(
        default_factory=KeywordConfig, metadata={"description": "Keyword configuration"}
    )
    percentiles: PercentileConfig = field(
        default_factory=PercentileConfig,
        metadata={"description": "Percentile configuration"},
    )
    fainder: FainderConfig = field(
        default_factory=FainderConfig, metadata={"description": "Fainder configuration"}
    )
    query_generation: QueryGenerationConfig = field(
        default_factory=QueryGenerationConfig,
        metadata={"description": "Query generation configuration"},
    )
    engines: EnginesConfig = field(
        default_factory=EnginesConfig, metadata={"description": "Engines configuration"}
    )
    experiment: ExperimentConfig = field(
        default_factory=ExperimentConfig,
        metadata={"description": "Experiment configuration"},
    )
    optimizer: OptimizerConfig = field(
        default_factory=OptimizerConfig,
        metadata={"description": "Optimizer configuration"},
    )
    ...
    def from_dict(cls, config_dict: Dict) -> "PerformanceConfig":
        ...
        percentiles = (
            PercentileConfig(**percentiles_dict)
            if percentiles_dict
            else PercentileConfig()
        )
        query_generation = (
            QueryGenerationConfig(**query_generation_dict)
            if query_generation_dict
            else QueryGenerationConfig()
        )
        ...
        experiment = (
            ExperimentConfig(**experiment_dict)
            if experiment_dict
            else ExperimentConfig()
        )
        fainder = FainderConfig(**fainder_dict) if fainder_dict else FainderConfig()
        optimizer = (
            OptimizerConfig(**optimizer_dict) if optimizer_dict else OptimizerConfig()
        )

        # Create the main config
        return cls(
            log_dir=config_dict.get("log_dir", "logs"),
            performance_log_dir=config_dict.get(

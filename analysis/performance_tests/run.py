...
from .eval_performance_test import (
...
def initialize_engines(config: PerformanceConfig) -> Dict[str, Engine]:
    ...
    fainder_index = FainderIndex(
        rebinning_path=settings.fainder_conversion_path_for_config("default"),
        conversion_path=settings.fainder_conversion_path_for_config("default"),
...
def create_csv_files(paths: Dict[str, Any]) -> Dict[str, Any]:
    ...
    perf_csv_path = (
        paths["all_results_dir"] / f"performance_metrics_{timestamp_str}.csv"
    )
    ...
    for mode in fainder_modes:
        for _ in range(num_runs):
            ...
            if not is_consistent:
                logger.warning(
                    f"Results inconsistent for {category} - {test_name} with query: {query}"
                )
